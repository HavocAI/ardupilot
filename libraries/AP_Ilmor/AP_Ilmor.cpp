...
/*
    ...
    Description:
    A Rover CAN driver to control the RPM of the Ilmor Ion motor via the
   "Throttle" servo function. Motor trim can be commanded on a user-definable
   servo function (see parameters below). Motor trim is set using a simple
   controller in the driver. If trim is not being actively commanded up or down
   by the controller, it will be relinquished to the physical trim buttons.
   Limited telemetry is available via the escX_ telemetry information, some of
   which is hacked in to the multiple ESC telemetry slots.

    Notes:
    1) Currently this class is a singleton for simplicity.

    Configuring CAN Parameters in ArduRover: First configure the CAN port to use
   the J1939 CAN backend (See AP_J1939_CAN/AP_J1939_CAN.cpp for more
   information)
...
void IlmorFwVersion::print() const {
  GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Ilmor: Fw v%d.%d.%d-%d.%d", major, minor,
                patch, dev_stage, dev_stage_rev);
}
...
bool IlmorFwVersion::operator<(const IlmorFwVersion &other) const {
  ...
  if (dev_stage != other.dev_stage)
    return dev_stage < other.dev_stage;
  return dev_stage_rev < other.dev_stage_rev;
}

MessageRateIIR::MessageRateIIR(float time_constant_sec)
    : _tau(time_constant_sec), _last_update_ms(0), _average_rate_hz(0.0f) {
  if (_tau <= 0.0f) {
    _tau = 5.0f;
  }
}
...
const AP_Param::GroupInfo AP_Ilmor::var_info[] = {
    ...
    // @Description: Ilmor Motor Trim Servo Channel, by default this is set to
    // "gripper" (28)
    // @Values: 0:255
    // @Increment: 1
    // @User: Advanced
    ...
    // @Description: Ilmor Motor Maximum Trim that the motor will be allowed to
    // run at
    // @Values: 0:127
    // @Increment: 1
    // @User: Advanced
    ...
    // @Description: Ilmor Firmware Update Server, 0 = off, 1 = on, 2 = request
    // to turn on
    // @Values: 0:2
    // @Increment: 1
    // @User: Advanced
    ...
    // @Description: Enable ICU logging, 0 = off, 1 = start logging, 2 = wipe
    // logs
    // @Values: 0:2
    // @Increment: 1
    // @User: Advanced
    ...
    // @Description: Automatically trim down every minute, 0 = off, 1 = on, 2 =
    // disable all trim
    // @Values: 0:1
    // @Increment: 1
    // @User: Advanced
    ...
    // @Description: The threshold for automatically trimming down, by default
    // this is set to 12
    // @Values: 0:255
    // @Increment: 1
    // @User: Advanced
    ...
    // @Description: The time period (ms) to automatically trim down, by default
    // this is set to 60000
    // @Increment: 1
    // @User: Advanced
...
AP_Ilmor::AP_Ilmor()
    : CANSensor("Ilmor"), _led_mode(LEDMode::Off), _trimState(TrimState::Start),
...
void AP_Ilmor::init(uint8_t driver_index, bool enable_filters) {
  ...
  hal.scheduler->register_io_process(
      FUNCTOR_BIND_MEMBER(&AP_Ilmor::tick, void));

  hal.util->snprintf(_thread_name, sizeof(_thread_name), "ilmor%d_tx",
                     driver_index);
  hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AP_Ilmor::run_io, void),
                               _thread_name, 1024,
...
void AP_Ilmor::send_throttle_cmd() {
  const uint8_t trim_upper_limit =
      _trim_stop.get() < 0 ? 255 : _trim_stop.get();

  if (AP_HAL::millis() - _run_state.last_send_throttle_ms >=
      1000 / AP_ILMOR_COMMAND_RATE_HZ) {
    ...
    if (!send_unmanned_throttle_control(throttle_msg)) {
      // GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "Failed to send throttle control
      // message");
    }
...
void AP_Ilmor::send_trim_cmd() {
  if (AP_HAL::millis() - _run_state.last_send_trim_ms >=
      1000 / AP_ILMOR_COMMAND_RATE_HZ) {

    ilmor_r3_status_frame_2_t r3_status_frame_2_msg = {
        .led_hue = _led_hue,
        .led_saturation = _led_hue == 255
                              ? static_cast<uint8_t>(0U)
                              : static_cast<uint8_t>(
                                    255U), // special handling for 255 (white),
                                           // otherwise full saturation
    ...
    if (!send_r3_status_frame_2(r3_status_frame_2_msg)) {
      // GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "Failed to send trim control
      // message");
    }
...
void AP_Ilmor::handle_frame(AP_HAL::CANFrame &frame) {
  ...
  default: {
    ...
    if (!is_from_icu) {
      // GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "Ilmor: pgn: %" PRIu32 " from
      // unexpected sa: %" PRIu8, pgn, id.source_address());
      break;
    } else {
    ...
    case J1939_PGN_DM1: {
      J1939::DiagnosticMessage1::DTC dtc =
          J1939::DiagnosticMessage1::DTC::from_data(&frame.data[2]);
    ...
    case J1939_PGN_TP_CM: {
      ...
      // GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "Ilmor: 0x%02X 0x%02X 0x%02X 0x%02X
...
bool AP_Ilmor::soft_stop_exceeded() {
  ...
  if (max_trim > 0 && 10 < _current_trim_position &&
      _current_trim_position < 254 && _current_trim_position > max_trim + 10) {
    return true;
  } else {
    return false;
  }
...
AP_Ilmor::TrimCmd AP_Ilmor::trim_demand() {
  ...
  SRV_Channel *channel = SRV_Channels::get_channel_for(
      (SRV_Channel::Aux_servo_function_t)_trim_fn.get());
  if (channel == nullptr) {
...
void AP_Ilmor::trim_state_machine() {
  ...
  case TrimState::Manual: {
    ...
    if (_auto_trim_down.get() == 1 &&
        _current_trim_position >
            static_cast<uint32_t>(_auto_trim_down_threshold.get()) &&
        AP_HAL::millis() - _last_auto_trim_down_ms >
            static_cast<uint32_t>(_auto_trim_down_period.get())) {
...
void AP_Ilmor::icu_logging_state_machine() {
  ...
  case ICULoggingState::Idle:
    ...
    case 2: {
      ...
      // Send a r3_status_frame_3 message with byte 2 set to 0x03. This will
      // clear the log.
  ...
  case ICULoggingState::StartLogging: {
    ...
    // Send a r3_status_frame_3 message with byte 2 set to 0x02 (all other bytes
    // 0x00). This will start a internal data log within the ICU.
    ilmor_r3_status_frame_3_t msg = {
  ...
  case ICULoggingState::StopLogging: {
    ...
    // Then send a r3_status_frame_3 message with byte 2 set to 0x01 (all other
    // bytes 0x00). This will stop logging.
    ilmor_r3_status_frame_3_t msg = {
...
bool AP_Ilmor::is_locked_out() {
  bool retval =
      !hal.util->get_soft_armed() || SRV_Channels::get_emergency_stop();

  // if the ICU is healthy, we also check the trim limit
  if (icu_healthy()) {
    retval |= (_max_run_trim.get() > 0 &&
               _current_trim_position > _max_run_trim.get());
  }
...
void AP_Ilmor::update() {
  const float throttle = constrain_float(
      SRV_Channels::get_output_norm(SRV_Channel::k_throttle), -1.0, 1.0);
...
void AP_Ilmor::report_faults() {
  ...
  for (int i = 0; i < _num_active_faults; i++) {
    ...
    GCS_SEND_TEXT(MAV_SEVERITY_ERROR,
                  "Ilmor: fault: SPN: %" PRIu32 " FMI: %" PRIu8 " oc: %" PRIu8,
                  dtc.spn(), dtc.fmi(), dtc.oc());
  }
}

bool AP_Ilmor::healthy() { return icu_healthy() && inverter_healthy(); }

bool AP_Ilmor::icu_healthy() { return _icu_msg_rate.rate_hz() > 5.0f; }

bool AP_Ilmor::inverter_healthy() {
  return _inverter_msg_rate.rate_hz() > 5.0f;
}

bool AP_Ilmor::send_unmanned_throttle_control(
    const struct ilmor_unmanned_throttle_control_t &msg) {
  // Prepare the data
  ...
  frame.pgn =
      J1939::extract_j1939_pgn(ILMOR_UNMANNED_THROTTLE_CONTROL_FRAME_ID);
  frame.source_address = AP_ILMOR_SOURCE_ADDRESS;
  memcpy(frame.data, data, sizeof(data));
...
bool AP_Ilmor::send_r3_status_frame_2(
    const struct ilmor_r3_status_frame_2_t &msg) {
...
void AP_Ilmor::handle_unmanned_throttle_control(
    const struct ilmor_unmanned_throttle_control_t &msg) {
  GCS_SEND_TEXT(
      MAV_SEVERITY_INFO,
      "Ilmor: Unmanned Throttle Control msg received from another CAN device");
}

void AP_Ilmor::handle_r3_status_frame_2(
    const struct ilmor_r3_status_frame_2_t &msg) {
  // GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Ilmor: R3 Status Frame 2 msg received
  // from another CAN device");
}

void AP_Ilmor::handle_icu_status_frame_1(
    const struct ilmor_icu_status_frame_1_t &msg) {
  _current_trim_position = msg.trim_position_adjusted;
  // GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Ilmor: Trim position %d",
  // msg.trim_position); GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Ilmor: Trim adj %d",
  // msg.trim_position_adjusted);

  // Populate esc2_rpm with the trim position
  update_rpm(1, msg.trim_position_adjusted);
}

void AP_Ilmor::handle_icu_status_frame_2(
    const struct ilmor_icu_status_frame_2_t &msg) {
  ...
  // if (counter++ % 100 == 0) {
  //     GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Ilmor: t: %" PRIi32 " s:%d
  //     v%d.%d.%d", msg.throttle_demand, msg.shift_position,
  //         msg.software_version_major, msg.software_version_minor,
  //         msg.software_version_patch);
  // }
}

void AP_Ilmor::handle_icu_status_frame_7(
    const struct ilmor_icu_status_frame_7_t &msg) {

  // GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Ilmor: trim demand %d",
  // msg.trim_demand_request_from_buttons); _trim_command_from_buttons =
  // msg.trim_demand_request_from_buttons; if (_trim_command_from_buttons != 0)
  // {
  //     // Operator is using buttons to adjust trim
  //     GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Ilmor: Trim buttons pressed");
  //     if (_trim_command_from_buttons == 1 && _current_trim_position >=
  //     AP_ILMOR_MAX_TRIM)
  //     {
  //         if (!_trim_locked_out)
  //         {
  //             GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Ilmor: Trim locked up by
  //             buttons");
  //         }
...
void AP_Ilmor::handle_inverter_status_frame_1(
    const struct ilmor_inverter_status_frame_1_t &msg) {
  // Divide by 5 to get Prop RPM.
  // Add 10000 because because the data reporting in mavlink is constrained to a
  // uint16_t type. We still want to be able to report negative RPM for reverse.
  ...
  update_telem_data(1, t, AP_ESC_Telem_Backend::TelemetryType::CURRENT);
}

void AP_Ilmor::handle_inverter_status_frame_2(
    const struct ilmor_inverter_status_frame_2_t &msg) {
  const TelemetryData t = {
      .consumption_mah = float((msg.ah_consumed) / 10.0f),
  };
  // Hack the current into the next+1 ESC telemetry slot
  update_telem_data(2, t, AP_ESC_Telem_Backend::TelemetryType::CURRENT);

  // Put the current consumption into our ESC telemetry slot
  // Note that this is not available via Mavlink, hence the use of the hack
  // above
  update_telem_data(0, t, AP_ESC_Telem_Backend::TelemetryType::CONSUMPTION);
}

void AP_Ilmor::handle_inverter_status_frame_3(
    const struct ilmor_inverter_status_frame_3_t &msg) {
  const TelemetryData t = {
      .voltage = float(msg.wh_consumed) / 10000.0f,
  };
  // Hack the Wh consumed into the next ESC telemetry slot
  update_telem_data(2, t, AP_ESC_Telem_Backend::TelemetryType::VOLTAGE);
}

void AP_Ilmor::handle_inverter_status_frame_4(
    const struct ilmor_inverter_status_frame_4_t &msg) {
  const TelemetryData t = {
  ...
  update_telem_data(1, t2, AP_ESC_Telem_Backend::TelemetryType::TEMPERATURE);
}

void AP_Ilmor::handle_inverter_status_frame_5(
    const struct ilmor_inverter_status_frame_5_t &msg) {
  const TelemetryData t = {
      .voltage = float(msg.low_precision_battery_voltage) / 10.0f,
  };
  update_telem_data(0, t, AP_ESC_Telem_Backend::TelemetryType::VOLTAGE);
}
